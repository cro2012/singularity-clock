...
import { CatastropheScreen } from '@sc/ui';
import type { Locale } from '@sc/i18n';
import { MODEL_CONFIG } from '../model-config.ts';

export function Catastrophe({ locale, now }: { locale: Locale; now: number }) {
  return <CatastropheScreen config={MODEL_CONFIG} locale={locale} now={now} />;
}
