...
import { TriggersScreen } from '@sc/ui';
import type { Locale } from '@sc/i18n';
import { MODEL_CONFIG } from '../model-config.ts';

export function Triggers({ locale, now }: { locale: Locale; now: number }) {
  return <TriggersScreen config={MODEL_CONFIG} locale={locale} now={now} />;
}
