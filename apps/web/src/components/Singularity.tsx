...
import { SingularityScreen } from '@sc/ui';
import type { Locale } from '@sc/i18n';
import { MODEL_CONFIG } from '../model-config.ts';

export function Singularity({ locale, now }: { locale: Locale; now: number }) {
  return <SingularityScreen config={MODEL_CONFIG} locale={locale} now={now} />;
}
