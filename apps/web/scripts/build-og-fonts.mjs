...
/**
 * Только латиница: сайт одноязычный.
 *
 * Если появится язык с другой письменностью, подмножество надо добавить сюда
 * И дать ему ОТДЕЛЬНОЕ имя семейства в og.mts. Satori выбирает шрифт по имени
 * и внутри одного имени между файлами не переключается — одинаковые имена
 * дают «NO GLYPH» на всём неохваченном алфавите.
 */
const SUBSETS = [
  ['latin', 600],
  ['latin', 400],
